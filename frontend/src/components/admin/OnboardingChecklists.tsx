import { ListTable, PageTitle, TableHeader, Text, Button, FormControlLabel, Paragraph } from "@freee_jp/vibes";
...
const AdminMainTasks = () => {
  ...
  const [currentUser, setCurrentUser] = useState<{ name: string; role: number } | null>(null);
  ...
  const taskRows = mainTasks.map(task => ({
    cells: [
      ...
      { 
        value: format(new Date(task.duration), 'MM/dd/yyyy'),
      },
      {
        value: (
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex justify-center items-center backdrop-blur-md z-50">
            ...
            {subTasks.length === 0 ? (
                ...
                rows={subTasks.map(subTask => ({
                    ...
                    { value: format(new Date(subTask.duration), 'MM/dd/yyyy') },
