...
import {
  ...
  const ToDoPage = () => {
    ...
    const fetchAssignedMainTasks = async (checklistTitle: string, page: number) => {
      try {
    ...
    const handleSearchChecklistTitle = (value: string) => {
      ...
      fetchAssignedMainTasks(value, currentPage);
    };
  
    const handlePageChange = (page: number) => {
      setCurrentPage(page);
      fetchAssignedMainTasks(searchChecklistTitle, page);
    };
    ...
    const assignedMainTaskRows = assignedMainTasks.map((task) => {
        ...
        return {
          cells: [
            ...
            { value: task.main_task_description },
            {
              value: (
                <div>
                  {status === "Not Started" && (
    ...
    return (
      <div>
        ...
        <Stack direction="horizontal" ma={1}>
        ...
        <TaskDialog
          ...
          title={`Upload Submissions for ${selectedMainTask?.main_task_name}`}
        ...
        <TaskDialog
          ...
          title={`Upload Submissions for ${selectedSubTask?.sub_task_name}`}
        ...
        <TaskDialog
          ...
          title={`Are you sure you want to complete ${selectedSubTask?.sub_task_name}?`}
          ...
          shouldCloseOnOverlayClickOrEsc={true}
        >
          <SectionTitle textAlign="center">You will need to ask the admin to revert this completion.</SectionTitle>
        </TaskDialog>
        <TaskDialog
          id="confirm-main-task-dialog"
          isOpen={isConfirmMainTaskDialogOpen}
          title={`Are you sure you want to complete ${selectedMainTask?.main_task_name}?`}
